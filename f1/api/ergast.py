...
async def get_driver_standings(season, rnd=None):
    ...
    if standings:
        ...
        for standing in standings['MRData']['StandingsTable']['StandingsLists'][0]['DriverStandings']:
            driver_info = {
                'Pos': standing['positionText'],
                'Driver': f"{standing['Driver']['givenName'][0]} {standing['Driver']['familyName']}",
...
async def get_team_standings(season, rnd=None):
    ...
    if standings:
        ...
        for standing in standings['MRData']['StandingsTable']['StandingsLists'][0]['ConstructorStandings']:
            results['data'].append(
                {
                    'Pos': standing['positionText'],
...
async def get_race_schedule(year):
    ...
    `MissingDataError`
        if API response unavailable.
    """
    url = f'{BASE_URL}/{str(year)}'
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            standings = await response.json()
            standings = standings['MRData']['RaceTable']
    if standings:
            ...
            res['data'].append(
                {
                    ...
                    'Points': float(standing.driverstanding['points']),
